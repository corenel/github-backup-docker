...
4. Set `GITHUB_USER`, `GITHUB_ORG_USER` (for GitHub Orgnization) and `GITHUB_ACCESS_TOKEN` (in environment or directly in `docker-compose.yml`). If you have a multiple users/organizations, you can list them in `GITHUB_USER`/`GITHUB_ORG_USER` variable separating names by a comma.
